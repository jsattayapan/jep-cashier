...
router.get('/payment/:id', authentication, (req, res) => {
...
router.post('/complete-payment', authentication, (req, res) => {
  const {total_amount, receive_amount, method, table_id, user_id, room_number, creditCardType, creditCardNumber} = req.body;
  db.knex.select('tables.outlet').from('customer_tables').leftJoin('tables', 'tables.number', 'customer_tables.table_number').where({id: table_id}).then((outletData) => {
    completePayment({total_amount, receive_amount, method, table_id, user_id, res, room_number, outlet: outletData[0].outlet, creditCardType, creditCardNumber});
  })
})


router.post('/void-payment', authentication, (req, res) => {
          ...
          db.knex('payment').where({customer_table_id}).then((returnData) => {
            if(returnData.length === 0){
              db.knex('payment').insert({
                customer_table_id, sub_total_amount : total, discount_amount: total_discount
              }).then(() => {
              ...
              db.knex('payment').where({customer_table_id}).update({ sub_total_amount : total, discount_amount: total_discount
              }).then(() => {
                db.knex('payment').where({customer_table_id}).then((rawData) => {
                  var payment_id = rawData[0].id;
                  db.knex('printers').where({name: 'แคชเชียร์'}).then((printerData) => {
...
const completePayment = ({total_amount, receive_amount, method, table_id, user_id, res, room_number = null, outlet, creditCardType = null, creditCardNumber = null}) => {

  const paymentInfo = method === 'card' ? `${creditCardType} ${creditCardNumber}` : room_number;

  db.knex('payment').where({customer_table_id: table_id}).update({total_amount, receive_amount, method, room_number: paymentInfo}).then(() => {
               ...
               db.knex('staffs').leftJoin('employees', 'employees.id' , 'staffs.id').where('staffs.id', '=', user_id).then((rawData) => {
                   db.knex('customer_table_discount').where({customer_table_id: table_id}).then((discountData) => {
                     getOrders(table_id, outlet, (ordersData) => {
                      ...
                      const total = sub_total - total_discount;
                      db.knex('printers').where({name: 'แคชเชียร์'}).then((printerData) => {
                        const printerIp = printerData[0].ip_address;
